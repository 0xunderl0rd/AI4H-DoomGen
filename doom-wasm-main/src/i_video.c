...
static boolean MouseShouldBeGrabbed()
{
    ...
    // Pointer-lock transitions in the browser path are a frequent source of
    // hangs and input stalls. Keep the web build keyboard-only for now.
    return false;
#endif
...
void I_StartTic (void)
{
    ...
    {
#ifdef __EMSCRIPTEN__
        // Relative mouse input is disabled in the browser build.
#else
        I_ReadMouse();
#endif
    }
...
{
    static int lasttic;
    static int emscripten_finishupdate_logs;
    int tics;
    int i;
...
#ifdef __EMSCRIPTEN__
    if (emscripten_finishupdate_logs < 0)
    {
        printf("I_FinishUpdate: before clear\n");
    }
#endif
    ...
    if (emscripten_finishupdate_logs < 0)
    {
        printf("I_FinishUpdate: after clear\n");
    }
#endif

    // The browser renderer path is more stable when we avoid the extra
    // render-target/upscaled-texture pass and present the streaming texture
    // directly to the canvas.
#ifdef __EMSCRIPTEN__
    if (emscripten_finishupdate_logs < 0)
    {
        printf("I_FinishUpdate: before direct render copy\n");
    }
    SDL_RenderCopy(renderer, texture, NULL, NULL);
    if (emscripten_finishupdate_logs < 0)
    {
        printf("I_FinishUpdate: after direct render copy\n");
    }
#else
    ...
    if (emscripten_finishupdate_logs < 0)
    {
        printf("I_FinishUpdate: before present\n");
    }
#endif
    ...
    if (emscripten_finishupdate_logs < 0)
    {
        printf("I_FinishUpdate: after present\n");
        ++emscripten_finishupdate_logs;
    }
#endif
...
static void SetVideoMode(void)
{
    ...
    w = window_width;
    h = window_height;
    printf("SetVideoMode: begin %d x %d\n", w, h);
    ...
    printf("SetVideoMode: flags prepared fullscreen=%d\n", fullscreen);
    ...
    printf("SetVideoMode: window position %d,%d\n", x, y);
    ...
    if (screen == NULL)
    {
        printf("SetVideoMode: creating window\n");
        ...
        printf("SetVideoMode: window created pixel_format=%u\n", pixel_format);
    ...
    printf("SetVideoMode: renderer flags base=%d\n", renderer_flags);

#ifdef __EMSCRIPTEN__
    // The browser path no longer uses render targets/upscaled intermediate
    // textures. Prefer the software renderer to avoid WebGL/canvas driver
    // instability once gameplay starts.
    renderer_flags = SDL_RENDERER_SOFTWARE;
    printf("SetVideoMode: emscripten renderer flags=%d\n", renderer_flags);
#else
    ...
    printf("SetVideoMode: display mode refresh=%d\n", mode.refresh_rate);
    ...
    printf("SetVideoMode: creating renderer flags=%d\n", renderer_flags);
    ...
    if (renderer == NULL && !force_software_renderer)
    {
        ...
        renderer = SDL_CreateRenderer(screen, -1, renderer_flags);
        printf("SetVideoMode: fallback renderer flags=%d success=%d\n", renderer_flags, renderer != NULL);
    ...
    printf("SetVideoMode: renderer created\n");
    ...
    printf("SetVideoMode: logical size configured\n");

    // Force integer scales for resolution-independent rendering.

    SDL_RenderSetIntegerScale(renderer, integer_scaling);
    printf("SetVideoMode: integer scale=%d\n", integer_scaling);
    ...
    SDL_RenderClear(renderer);
    SDL_RenderPresent(renderer);
    printf("SetVideoMode: renderer primed\n");
    ...
    if (screenbuffer == NULL)
    {
        printf("SetVideoMode: creating screenbuffer\n");
        screenbuffer = SDL_CreateRGBSurface(0,
                                            SCREENWIDTH, SCREENHEIGHT, 8,
                                            0, 0, 0, 0);
        SDL_FillRect(screenbuffer, NULL, 0);
    }
    printf("SetVideoMode: screenbuffer ready\n");
    ...
    if (argbbuffer == NULL)
    {
        ...
        printf("SetVideoMode: argb masks bpp=%d\n", bpp);
        argbbuffer = SDL_CreateRGBSurface(0,
                                          SCREENWIDTH, SCREENHEIGHT, bpp,
                                          rmask, gmask, bmask, amask);
        SDL_FillRect(argbbuffer, NULL, 0);
    }
    printf("SetVideoMode: argbbuffer ready\n");
    ...
    printf("SetVideoMode: scale quality set\n");
    ...
    printf("SetVideoMode: streaming texture created\n");

    // Initially create the upscaled texture for rendering to screen.
#ifndef __EMSCRIPTEN__
    CreateUpscaledTexture(true);
    printf("SetVideoMode: upscaled texture created\n");
#endif
}
...
void I_InitGraphics(void)
{
    ...
    SetSDLVideoDriver();
    printf("I_InitGraphics: driver set\n");

    if (SDL_Init(SDL_INIT_VIDEO) < 0) 
    {
        I_Error("Failed to initialize video: %s", SDL_GetError());
    }
    printf("I_InitGraphics: SDL video initialized\n");
    ...
    printf("I_InitGraphics: actualheight=%d fullscreen=%d\n", actualheight, fullscreen);

    // Create the game window; this may switch graphic modes depending
    // on configuration.
    AdjustWindowSize();
    printf("I_InitGraphics: window adjusted %d x %d\n", window_width, window_height);
    SetVideoMode();
    printf("I_InitGraphics: video mode set\n");

    // Start with a clear black screen
    // (screen will be flipped after we set the palette)

    SDL_FillRect(screenbuffer, NULL, 0);
    printf("I_InitGraphics: screenbuffer cleared\n");

    // Set the palette

    doompal = W_CacheLumpName(DEH_String("PLAYPAL"), PU_CACHE);
    printf("I_InitGraphics: palette lump cached\n");
    I_SetPalette(doompal);
    printf("I_InitGraphics: palette staged\n");
    SDL_SetPaletteColors(screenbuffer->format->palette, palette, 0, 256);
    printf("I_InitGraphics: palette uploaded\n");

    // SDL2-TODO UpdateFocus();
    UpdateGrab();
    printf("I_InitGraphics: grab updated\n");
    ...
    printf("I_InitGraphics: video buffer assigned\n");
    V_RestoreBuffer();
    printf("I_InitGraphics: buffer restored\n");

    // Clear the screen to black.

    memset(I_VideoBuffer, 0, SCREENWIDTH * SCREENHEIGHT * sizeof(*I_VideoBuffer));
    printf("I_InitGraphics: buffer zeroed\n");

    // clear out any events waiting at the start and center the mouse
  
    while (SDL_PollEvent(&dummy));
    printf("I_InitGraphics: events drained\n");
