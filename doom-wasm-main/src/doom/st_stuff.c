...
static patch_t*         stock_faces[ST_NUMFACES];
...
//
void ST_Stop(void);
static void ST_applyRuntimeFaceOverrides(void);
...
static void ST_applyRuntimeFaceOverrides(void)
{
    ...
    {
        patch_t *runtime_patch = (patch_t *) DoomGen_GetHudFaceRuntimePatch(i);
        int runtime_patch_length = DoomGen_GetHudFaceRuntimePatchLength(i);

        if (runtime_patch != NULL
            && runtime_patch_length > 0
            && DoomGen_IsValidRuntimePatchBytes(runtime_patch, runtime_patch_length))
        {
            faces[i] = runtime_patch;
        }
        else
        {
            faces[i] = stock_faces[i];
        }
    }
}
...
static void ST_loadUnloadGraphics(load_callback_t callback)
{
	...
	for (j=0; j<ST_NUMSTRAIGHTFACES; j++)
	{
	    DEH_snprintf(namebuf, 9, "STFST%d%d", i, j);
            ...
            stock_faces[facenum] = faces[facenum];
            ++facenum;
	}
