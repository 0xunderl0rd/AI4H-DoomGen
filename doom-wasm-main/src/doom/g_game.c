...
void G_BuildTiccmd(ticcmd_t *cmd, int maketic)
{
    ...
    forward = DoomGen_ScaleMoveInput(forward);
    side = DoomGen_ScaleMoveInput(side);
...
void G_DoLoadLevel(void)
{
    ...
    // Guard against invalid episode/map globals before resolving map lumps.
    // Browser input/runtime edge cases can occasionally request episode 0.
    ...
    else
    {
        ...
        if (gamemode == shareware && gameepisode > 1)
        {
            gameepisode = 1;
        }
...
void G_Ticker(void)
{
    ...
    static int emscripten_gticker_logs = 0;
#endif
    ...
    case GS_LEVEL:
#ifdef __EMSCRIPTEN__
        if (emscripten_gticker_logs < 0) {
            printf("G_Ticker: before P_Ticker gametic=%d leveltime=%d\n", gametic, leveltime);
        }
#endif
        P_Ticker();
#ifdef __EMSCRIPTEN__
        if (emscripten_gticker_logs < 0) {
            printf("G_Ticker: after P_Ticker gametic=%d leveltime=%d\n", gametic, leveltime);
            printf("G_Ticker: before ST_Ticker gametic=%d leveltime=%d\n", gametic, leveltime);
        }
#endif
        ST_Ticker();
#ifdef __EMSCRIPTEN__
        if (emscripten_gticker_logs < 0) {
            printf("G_Ticker: after ST_Ticker gametic=%d leveltime=%d\n", gametic, leveltime);
            printf("G_Ticker: before AM_Ticker gametic=%d leveltime=%d\n", gametic, leveltime);
        }
#endif
        AM_Ticker();
#ifdef __EMSCRIPTEN__
        if (emscripten_gticker_logs < 0) {
            printf("G_Ticker: after AM_Ticker gametic=%d leveltime=%d\n", gametic, leveltime);
            printf("G_Ticker: before HU_Ticker gametic=%d leveltime=%d\n", gametic, leveltime);
        }
#endif
        HU_Ticker();
#ifdef __EMSCRIPTEN__
        if (emscripten_gticker_logs < 0) {
            printf("G_Ticker: after HU_Ticker gametic=%d leveltime=%d\n", gametic, leveltime);
            ++emscripten_gticker_logs;
        }
#endif
        break;
...
{
#ifdef __EMSCRIPTEN__
    printf("G_DoNewGame: pre reset playeringame={%d,%d,%d,%d} demoplayback=%d\n",
           playeringame[0], playeringame[1], playeringame[2], playeringame[3], demoplayback);
#endif
    demoplayback = false;
    netdemo = false;
    netgame = false;
    ...
    consoleplayer = 0;
#ifdef __EMSCRIPTEN__
    printf("G_DoNewGame: post reset playeringame={%d,%d,%d,%d}\n",
           playeringame[0], playeringame[1], playeringame[2], playeringame[3]);
    printf("G_DoNewGame: addr playeringame2=%p players0mo=%p players2mo=%p\n",
           (void *) &playeringame[2], (void *) &players[0].mo, (void *) &players[2].mo);
    printf("G_DoNewGame: sizeof(boolean)=%d sizeof(playeringame[0])=%d\n",
           (int) sizeof(boolean), (int) sizeof(playeringame[0]));
#endif
