...
static void D_UpdateBrowserState(void)
{
    ...
    mobj_t *player_mobj;
    mobj_t *player0_mobj;
    pspdef_t *weapon_psprite;
    pspdef_t *flash_psprite;
    state_t *weapon_state;
    state_t *flash_state;
    int player_x;
    int player_y;
    ...
    int flash_sprite;
    int flash_frame;
    int weapon_psprite_suppressed;

    player = &players[consoleplayer];
    player0 = &players[0];
    player_mobj = player->mo;
    player0_mobj = player0->mo;
    weapon_psprite = &player->psprites[ps_weapon];
    flash_psprite = &player->psprites[ps_flash];
    weapon_state = weapon_psprite->state;
    flash_state = flash_psprite->state;
    player_x = 0;
    player_y = 0;
    player_angle = 0;
    ...
    if (player_mobj != NULL)
    {
        player_x = player_mobj->x;
        player_y = player_mobj->y;
        player_angle = player_mobj->angle;
    }

    ammo_clip = player->ammo[am_clip];
    readyweapon = player->readyweapon;
    pendingweapon = player->pendingweapon;
    attackdown = player->attackdown;
    refire = player->refire;
    weapon_state_id = weapon_state != NULL ? (int) (weapon_state - states) : -1;
    weapon_sprite = weapon_state != NULL ? weapon_state->sprite : -1;
    weapon_frame = weapon_state != NULL ? weapon_state->frame : -1;
    weapon_sx = weapon_psprite->sx;
    weapon_sy = weapon_psprite->sy;
    flash_state_id = flash_state != NULL ? (int) (flash_state - states) : -1;
    flash_sprite = flash_state != NULL ? flash_state->sprite : -1;
    flash_frame = flash_state != NULL ? flash_state->frame : -1;
    weapon_psprite_suppressed = DoomGen_ShouldHideWeaponPsprites(readyweapon, pendingweapon);

    EM_ASM({
        if (typeof window !== "undefined")
        {
            window.__doomStandaloneState = window.__doomStandaloneState || {};
            window.__doomStandaloneState.gameplay = window.__doomStandaloneState.gameplay || {};
            window.__doomStandaloneState.gameplay.gamestate = $0;
            window.__doomStandaloneState.gameplay.gameepisode = $1;
            window.__doomStandaloneState.gameplay.gamemap = $2;
            window.__doomStandaloneState.gameplay.leveltime = $3;
            window.__doomStandaloneState.gameplay.hasPlayer = !!$4;
            window.__doomStandaloneState.gameplay.playerX = $5;
            window.__doomStandaloneState.gameplay.playerY = $6;
            window.__doomStandaloneState.gameplay.playerAngle = $7 >>> 0;
            window.__doomStandaloneState.gameplay.ammoClip = $8;
            window.__doomStandaloneState.gameplay.consoleplayer = $9;
            window.__doomStandaloneState.gameplay.player0HasMo = !!$10;
        }
    }, gamestate, gameepisode, gamemap, leveltime, player_mobj != NULL, player_x, player_y, player_angle,
       ammo_clip, consoleplayer, player0_mobj != NULL);
    ...
    EM_ASM({
        if (typeof window !== "undefined")
        {
            window.__doomStandaloneState = window.__doomStandaloneState || {};
            window.__doomStandaloneState.gameplay = window.__doomStandaloneState.gameplay || {};
            window.__doomStandaloneState.gameplay.readyweapon = $0;
            window.__doomStandaloneState.gameplay.pendingweapon = $1;
            window.__doomStandaloneState.gameplay.attackdown = !!$2;
            window.__doomStandaloneState.gameplay.refire = $3;
            window.__doomStandaloneState.gameplay.weaponState = $4;
            window.__doomStandaloneState.gameplay.weaponSprite = $5;
            window.__doomStandaloneState.gameplay.weaponFrame = $6;
            window.__doomStandaloneState.gameplay.weaponSx = $7;
            window.__doomStandaloneState.gameplay.weaponSy = $8;
            window.__doomStandaloneState.gameplay.flashState = $9;
            window.__doomStandaloneState.gameplay.flashSprite = $10;
            window.__doomStandaloneState.gameplay.flashFrame = $11;
            window.__doomStandaloneState.gameplay.weaponPspriteSuppressed = !!$12;
        }
    }, readyweapon, pendingweapon, attackdown, refire, weapon_state_id, weapon_sprite, weapon_frame,
       weapon_sx, weapon_sy, flash_state_id, flash_sprite, flash_frame, weapon_psprite_suppressed);
#endif
}
...
boolean D_Display(void)
{
    ...
    static int emscripten_level_display_logs = 0;
    int y;
    boolean wipe;
    boolean redrawsbar;
...
#ifdef __EMSCRIPTEN__
        if (emscripten_level_display_logs < 0) {
            printf("D_Display: before ST_Drawer gametic=%d leveltime=%d viewheight=%d fullscreen=%d\n",
                   gametic, leveltime, viewheight, fullscreen);
        }
#endif
        ST_Drawer(viewheight == SCREENHEIGHT, redrawsbar);
#ifdef __EMSCRIPTEN__
        if (emscripten_level_display_logs < 0) {
            printf("D_Display: after ST_Drawer gametic=%d leveltime=%d\n", gametic, leveltime);
        }
#endif
...
#ifdef __EMSCRIPTEN__
        if (emscripten_level_display_logs < 0) {
            printf("D_Display: before R_RenderPlayerView gametic=%d leveltime=%d displayplayer=%d\n",
                   gametic, leveltime, displayplayer);
        }
#endif
        R_RenderPlayerView(&players[displayplayer]);
#ifdef __EMSCRIPTEN__
        if (emscripten_level_display_logs < 0) {
            printf("D_Display: after R_RenderPlayerView gametic=%d leveltime=%d\n", gametic, leveltime);
            ++emscripten_level_display_logs;
        }
#endif
    }
...
void D_RunFrame()
{
    ...
    static boolean wipe;
#ifdef __EMSCRIPTEN__
    static int emscripten_runframe_logs = 0;

    if (emscripten_runframe_logs < 0) {
        printf("D_RunFrame: entry gamestate=%d gametic=%d leveltime=%d wipe=%d\n",
               gamestate, gametic, leveltime, wipe);
    }
#endif
...
#ifdef __EMSCRIPTEN__
    if (emscripten_runframe_logs < 0) {
        printf("D_RunFrame: before TryRunTics gamestate=%d gametic=%d\n", gamestate, gametic);
    }
#endif
    TryRunTics(); // will run at least one tic
#ifdef __EMSCRIPTEN__
    if (emscripten_runframe_logs < 0) {
        printf("D_RunFrame: after TryRunTics gamestate=%d gametic=%d\n", gamestate, gametic);
        printf("D_RunFrame: before S_UpdateSounds\n");
    }
#endif
    D_UpdateBrowserState();

    S_UpdateSounds(players[consoleplayer].mo); // move positional sounds
#ifdef __EMSCRIPTEN__
    if (emscripten_runframe_logs < 0) {
        printf("D_RunFrame: after S_UpdateSounds\n");
    }
#endif
...
#ifdef __EMSCRIPTEN__
        if (emscripten_runframe_logs < 0) {
            printf("D_RunFrame: before D_Display\n");
        }
#endif
        if ((wipe = D_Display())) {
...
#ifdef __EMSCRIPTEN__
    if (emscripten_runframe_logs < 0) {
        printf("D_RunFrame: after D_Display\n");
        ++emscripten_runframe_logs;
    }
#endif
}
...
void D_DoomLoop(void)
{
    ...
    if (demorecording) {
        printf("D_DoomLoop: demorecording\n");
        G_BeginRecording();
    }
    ...
    I_InitGraphics();
    printf("D_DoomLoop: graphics initialized\n");
    EnableLoadingDisk();
...
#endif
    R_ExecuteSetViewSize();
    printf("D_DoomLoop: view size finalized\n");

    D_StartGameLoop(); // it's not a loop, just an initializer
    printf("D_DoomLoop: timing initialized\n");

    if (testcontrols) {
        wipegamestate = gamestate;
    }
    printf("D_DoomLoop: pre main loop state=%d testcontrols=%d\n", gamestate, testcontrols);

    // Register the browser main loop after engine/video init is complete.
    // Use simulate_infinite_loop=0 so callMain() can return and the page
    // remains responsive for telemetry and external runtime hooks.
    ...
    EM_ASM({
        ...
        {
            window.__doomStandaloneState = window.__doomStandaloneState || {};
            window.__doomStandaloneState.mainStarted = true;
        }
    });
    printf("D_DoomLoop: mainStarted flag set\n");
#endif
    emscripten_set_main_loop(D_RunFrame, 0, 0);
    printf("D_DoomLoop: emscripten_set_main_loop returned\n");
}
...
void D_DoomMain(void)
{
    ...
    printf("D_DoomMain: after I_AtExit\n");
    ...
    printf("D_DoomMain: after I_CheckIsScreensaver\n");
    I_InitTimer();
    printf("D_DoomMain: after I_InitTimer\n");
    I_InitJoystick();
    printf("D_DoomMain: after I_InitJoystick\n");
    I_InitSound(true);
    printf("D_DoomMain: after I_InitSound\n");
    I_InitMusic();
    printf("D_DoomMain: after I_InitMusic\n");

    printf("NET_Init: Init network subsystem.\n");
    NET_Init();
    printf("D_DoomMain: after NET_Init\n");

    // Initial netgame startup. Connect to server etc.
    D_ConnectNetGame();
    printf("D_DoomMain: after D_ConnectNetGame netgame=%d\n", netgame);
    ...
    printf("D_DoomMain: after S_Init\n");

    D_CheckNetGame();
    printf("D_DoomMain: after D_CheckNetGame netgame=%d\n", netgame);

    PrintGameVersion();

    HU_Init();
    printf("D_DoomMain: after HU_Init\n");

    ST_Init();
    printf("D_DoomMain: after ST_Init\n");
    ...
    printf(
        "D_DoomMain: entering D_DoomLoop with gamestate=%d gameaction=%d autostart=%d netgame=%d\n",
        gamestate,
        gameaction,
        autostart,
        netgame
