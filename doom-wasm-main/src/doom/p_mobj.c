...
void P_ZMovement (mobj_t* mo)
{
    ...
    gravity = DoomGen_GetGravityValue();
...
void P_SpawnPlayer (mapthing_t* mthing)
{
    ...
    static int emscripten_spawnplayer_logs = 0;
#endif
...
#ifdef __EMSCRIPTEN__
    if (emscripten_spawnplayer_logs < 8)
    {
        printf("P_SpawnPlayer: type=%d playeringame0=%d playeringame1=%d state0=%d\n",
               mthing->type,
               playeringame[0],
               playeringame[1],
    ...
    if (emscripten_spawnplayer_logs < 8)
    {
        printf("P_SpawnPlayer: assigned mo=%p consoleplayer=%d\n", (void *) p->mo, consoleplayer);
        ++emscripten_spawnplayer_logs;
    }
#endif
}
  ...
  mobjtype_t	type )
{
    ...
    th->angle = an;
    an >>= ANGLETOFINESHIFT;
    missile_speed = DoomGen_ScaleProjectileSpeed(th->info->speed);
    if (missile_speed < FRACUNIT / 8)
    {
        missile_speed = FRACUNIT / 8;
    }
    th->momx = FixedMul (missile_speed, finecosine[an]);
    th->momy = FixedMul (missile_speed, finesine[an]);
	
    dist = P_AproxDistance (dest->x - source->x, dest->y - source->y);
    dist = dist / missile_speed;
  ...
  mobjtype_t	type )
{
    ...
    th->momx = FixedMul( missile_speed,
			 finecosine[an>>ANGLETOFINESHIFT]);
    th->momy = FixedMul( missile_speed,
			 finesine[an>>ANGLETOFINESHIFT]);
    th->momz = FixedMul( missile_speed, slope);
