...
static int DoomGen_WeaponTypeForSfx(sfxenum_t sfx_id)
{
    switch (sfx_id)
    {
        case sfx_punch:
...
void S_StartSound(void *origin_p, int sfx_id)
{
    ...
    // Weapon override playback hook for generated assets.
    {
        int weapon_type = DoomGen_WeaponTypeForSfx(sfx_id);
        if (weapon_type != wp_nochange)
        {
            const char *pack = DoomGen_GetWeaponSoundPack(weapon_type);
    ...
    if (DoomGen_PlayBuiltinSfx(sfx->name, volume, sep))
    {
        return;
    }
...
void S_ChangeMusic(int musicnum, int looping)
{
    ...
    if (musicnum <= mus_None || musicnum >= NUMMUSIC)
    {
        ...
        fprintf(stderr, "Ignoring invalid music number %d\n", musicnum);
        return;
#else
