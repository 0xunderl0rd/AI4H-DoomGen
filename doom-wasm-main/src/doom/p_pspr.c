  ...
  statenum_t	stnum ) 
{
	...
	state = &states[stnum];
	psp->state = state;
        is_pistol = position == ps_weapon
                    && (player->readyweapon == wp_pistol
                        || player->pendingweapon == wp_pistol);
	psp->tics = DoomGen_ScaleWeaponTics(state->tics, is_pistol);	// could be 0
  ...
  pspdef_t*	psp ) 
{
    ...
    preset = DoomGen_GetPistolProjectilePreset();

    switch (preset)
    {
        case DOOMGEN_PROJECTILE_HOOK:
            P_SpawnPlayerMissile(player->mo, MT_PLASMA);
            break;
        case DOOMGEN_PROJECTILE_ARCING:
            P_SpawnPlayerMissile(player->mo, MT_ROCKET);
            break;
        case DOOMGEN_PROJECTILE_SPREAD:
            ...
            break;
        case DOOMGEN_PROJECTILE_PIERCING:
            P_BulletSlope(player->mo);
            for (i = 0; i < 2; ++i)
            {
            ...
            break;
        case DOOMGEN_PROJECTILE_BULLET:
        default:
            P_BulletSlope(player->mo);
