...
void P_LoadThings (int lump)
{
    ...
    static int emscripten_loadthings_logs = 0;
#endif
	...
	if (spawn == false)
        {
#ifdef __EMSCRIPTEN__
            if (emscripten_loadthings_logs < 4)
            {
                printf("P_LoadThings: skipping unsupported type=%d and breaking\n", SHORT(mt->type));
                ++emscripten_loadthings_logs;
            }
#endif
	    break;
        }
    ...
    if (!deathmatch)
    {
        ...
        {
#ifdef __EMSCRIPTEN__
            if (emscripten_loadthings_logs < 8)
            {
                printf("P_LoadThings: player=%d playeringame=%d playerstartsingame=%d deathmatch=%d\n",
                       i,
                       playeringame[i],
                       playerstartsingame[i],
                       deathmatch);
            }
#endif
