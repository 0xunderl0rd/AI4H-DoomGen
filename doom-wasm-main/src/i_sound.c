...
void I_InitSound(boolean use_sfx_prefix)
{
    ...
    browser_safe_audio = !M_ParmExists("-allow-browser-audio");
#else
    browser_safe_audio = false;
#endif

    if (browser_safe_audio) {
        printf("I_InitSound: browser-safe audio mode enabled; skipping SDL audio init.\n");
        nosound = true;
    }
