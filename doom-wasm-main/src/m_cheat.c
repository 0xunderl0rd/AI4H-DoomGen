  ...
  char		key )
{
    ...
    unsigned char normalized_key;

    normalized_key = (unsigned char) key;

    // Browser key events can deliver non-text control values for movement
    // and other gameplay keys. Ignore anything that is not a simple
    // alphanumeric character so those inputs cannot accidentally drive the
    // cheat parser.
    if (!isalnum(normalized_key))
    {
        return false;
    }

    key = (char) tolower(normalized_key);
#endif
