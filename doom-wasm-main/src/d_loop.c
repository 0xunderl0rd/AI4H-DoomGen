...
static int EffectiveTicdup(void)
{
    // The browser path can reach the main loop before the net setup path
    // has copied settings into the global ticdup. Never allow zero here.
    return ticdup > 0 ? ticdup : 1;
}
...
static boolean BuildNewTic(void)
{
    ...
    gameticdiv = gametic / EffectiveTicdup();
...
void NetUpdate(void)
{
    ...
    // check time
    nowtime = GetAdjustedTime() / EffectiveTicdup();
...
void D_StartGameLoop(void) { lasttime = GetAdjustedTime() / EffectiveTicdup(); }
...
void TryRunTics(void)
{
    ...
    // get real tics
    entertic = I_GetTime() / EffectiveTicdup();
    ...
    availabletics = lowtic - gametic / EffectiveTicdup();
    ...
    while (!PlayersInGame() || lowtic < gametic / EffectiveTicdup() + counts) {
#ifdef __EMSCRIPTEN__
        // Never block the browser main thread waiting for more tics.
        // Yield back to requestAnimationFrame and try again next frame.
        return;
#endif
        NetUpdate();
        ...
        if (lowtic < gametic / EffectiveTicdup()) I_Error("TryRunTics: lowtic < gametic");

        // Still no tics to run? Sleep until some are available.
        if (lowtic < gametic / EffectiveTicdup() + counts) {
            ...
            if (I_GetTime() / EffectiveTicdup() - entertic >= MAX_NETGAME_STALL_TICS) {
                return;
            }
    ...
    while (counts--) {
        ...
        set = &ticdata[(gametic / EffectiveTicdup()) % BACKUPTICS];

        if (!net_client_connected) {
            SinglePlayerClear(set);
        }

        for (i = 0; i < EffectiveTicdup(); i++) {
            if (gametic / EffectiveTicdup() > lowtic) I_Error("gametic>lowtic");
